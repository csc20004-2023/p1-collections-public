...
public class DataRecordDynArray extends DataRecordCollection {
    ...
    /** A helper method to create a bigger array once the original storage is full,
     * copy the content of the old array in the new one, and finally swap the two.
     *
     */
    private void resizeStorage() {
        DataRecord[] biggerStorage = new DataRecord[storage.length + BATCH_SIZE];

        // copy all previous data over to the new storage
        for (int i = 0; i < storage.length; i++) {
        ...
        for (int i = 0; i < BATCH_SIZE; i++) {
            ...
            biggerStorage[storage.length + i] = dr;
            numElements++;
        }
    ...
    public DataRecord next() {
      // check if we have reached the limit of the current array
